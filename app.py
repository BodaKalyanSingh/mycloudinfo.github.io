...
def upload_file():
    ...
    if file:
        ...
        # Map the predicted class to cloud details
        cloud_classes = list(cloud_details.keys())
        cloud_type_key = cloud_classes[predicted_class]
        cloud_info = cloud_details.get(cloud_type_key, {})
        cloud_info['filename'] = filename
        
        return jsonify(cloud_info)
